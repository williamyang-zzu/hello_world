...
import { getKeypairFromEnvironment, getKeypairFromFile, addKeypairToEnvFile, getCustomErrorMessage, airdropIfRequired, getExplorerLink, confirmTransaction, makeKeypairs, initializeKeypair, getLogs, getSimulationComputeUnits, createAccountsMintsAndTokenAccounts, makeTokenMint, } from "./index";
...
describe(`getCustomErrorMessage`, () => {
    test(`we turn error messages with hex codes into error messages for the program`, () => {
        ...
        const errorMessage = getCustomErrorMessage(programErrors, "failed to send transaction: Transaction simulation failed: Error processing Instruction 0: custom program error: 0x10");
...
describe("getKeypairFromEnvironment", () => {
    ...
    test("throws a nice error if the value of the env var isn't valid", () => {
        assert.throws(() => getKeypairFromEnvironment("TEST_ENV_VAR_WITH_BAD_VALUE"), {
...
describe("addKeypairToEnvFile", () => {
    ...
    test("throws a nice error if the env var already exists", async () => {
        assert.rejects(async () => addKeypairToEnvFile(testKeypair, TEST_ENV_VAR_ARRAY_OF_NUMBERS), {
...
describe("airdropIfRequired", () => {
    test("Checking the balance after airdropIfRequired", async () => {
        ...
        const newBalance = await airdropIfRequired(connection, keypair.publicKey, lamportsToAirdrop, 1 * LAMPORTS_PER_SOL);
        assert.equal(newBalance, lamportsToAirdrop);
        const recipient = Keypair.generate();
        // Spend our SOL now to ensure we can use the airdrop immediately
        await connection.sendTransaction(new Transaction().add(SystemProgram.transfer({
    ...
    test("doesn't request unnecessary airdrops", async () => {
        ...
        assert.equal(originalBalance, 0);
        const lamportsToAirdrop = 1 * LAMPORTS_PER_SOL;
        await airdropIfRequired(connection, keypair.publicKey, lamportsToAirdrop, 500000);
        const finalBalance = await airdropIfRequired(connection, keypair.publicKey, lamportsToAirdrop, 1 * LAMPORTS_PER_SOL);
    ...
    test("airdropIfRequired does airdrop when necessary", async () => {
        ...
        const finalBalance = await airdropIfRequired(connection, keypair.publicKey, 1 * LAMPORTS_PER_SOL, 1 * LAMPORTS_PER_SOL);
...
describe("getExplorerLink", () => {
    ...
    test("getExplorerLink works for an address on mainnet", () => {
        const link = getExplorerLink("address", "dDCQNnDmNbFVi8cQhKAgXhyhXeJ625tvwsunRyRc7c8", "mainnet-beta");
    ...
    test("getExplorerLink works for an address on devnet", () => {
        const link = getExplorerLink("address", "dDCQNnDmNbFVi8cQhKAgXhyhXeJ625tvwsunRyRc7c8", "devnet");
        assert.equal(link, "https://explorer.solana.com/address/dDCQNnDmNbFVi8cQhKAgXhyhXeJ625tvwsunRyRc7c8?cluster=devnet");
    });
    test("getExplorerLink works for a transaction on mainnet", () => {
        const link = getExplorerLink("transaction", "4nzNU7YxPtPsVzeg16oaZvLz4jMPtbAzavDfEFmemHNv93iYXKKYAaqBJzFCwEVxiULqTYYrbjPwQnA1d9ZCTELg", "mainnet-beta");
        assert.equal(link, "https://explorer.solana.com/tx/4nzNU7YxPtPsVzeg16oaZvLz4jMPtbAzavDfEFmemHNv93iYXKKYAaqBJzFCwEVxiULqTYYrbjPwQnA1d9ZCTELg");
    });
    test("getExplorerLink works for a block on mainnet", () => {
    ...
    test("getExplorerLink provides a localnet URL", () => {
        const link = getExplorerLink("tx", "2QC8BkDVZgaPHUXG9HuPw7aE5d6kN5DTRXLe2inT1NzurkYTCFhraSEo883CPNe18BZ2peJC1x1nojZ5Jmhs94pL", "localnet");
        assert.equal(link, "https://explorer.solana.com/tx/2QC8BkDVZgaPHUXG9HuPw7aE5d6kN5DTRXLe2inT1NzurkYTCFhraSEo883CPNe18BZ2peJC1x1nojZ5Jmhs94pL?cluster=custom&customUrl=http%3A%2F%2Flocalhost%3A8899");
    });
});
...
describe("confirmTransaction", () => {
    test("confirmTransaction works for a successful transaction", async () => {
        ...
        await airdropIfRequired(connection, sender.publicKey, lamportsToAirdrop, 1 * LAMPORTS_PER_SOL);
        const transaction = await connection.sendTransaction(new Transaction().add(SystemProgram.transfer({
...
describe("getSimulationComputeUnits", () => {
    test("getSimulationComputeUnits returns 300 CUs for a SOL transfer, and 3888 for a SOL transfer with a memo", async () => {
        ...
        const computeUnitsSendSol = await getSimulationComputeUnits(connection, [sendSol], sender.publicKey, []);
        // TODO: it would be useful to have a breakdown of exactly how 300 CUs is calculated
        assert.equal(computeUnitsSendSol, 300);
        const computeUnitsSendSolAndSayThanks = await getSimulationComputeUnits(connection, [sendSol, sayThanks], sender.publicKey, []);
...
describe("makeTokenMint", () => {
    test("makeTokenMint makes a new mint with the specified metadata", async () => {
        ...
        const connection = new Connection(LOCALHOST);
        await airdropIfRequired(connection, mintAuthority.publicKey, 100 * LAMPORTS_PER_SOL, 1 * LAMPORTS_PER_SOL);
        ...
        const mintAddress = await makeTokenMint(connection, mintAuthority, name, symbol, decimals, uri, additionalMetadata);
        ...
        assert.equal((_a = tokenMetadata.updateAuthority) === null || _a === void 0 ? void 0 : _a.toBase58(), mintAuthority.publicKey.toBase58());
...
describe("createAccountsMintsAndTokenAccounts", () => {
    test("createAccountsMintsAndTokenAccounts works", async () => {
        ...
        const connection = new Connection(LOCALHOST);
        await airdropIfRequired(connection, payer.publicKey, 100 * LAMPORTS_PER_SOL, 1 * LAMPORTS_PER_SOL);
        ...
        assert(Number(firstUserFirstTokenBalance.value.amount) === 1000000000);
        ...
        assert(Number(secondUserSecondTokenBalance.value.amount) === 1000000000);
    });
});
